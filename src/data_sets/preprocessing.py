...
def train_test_split(X: np.ndarray, y: np.ndarray, proportion: float = 0.6):
    """Splits data and labels into train and test data and labels, respectively.
    ...
    Args:
        ...
        proportion: float, Optional
            The proportion of the data and labels that is used for training. (Default is 0.6.)
...
def min_max_feature_scaling(X_train: np.ndarray, X_test: np.ndarray):
    """Apply min-max feature scaling based on X_train to both X_train and X_test.
...
def split_into_classes(X: np.ndarray, y: np.ndarray):
    """Splits the data X into its classes, which allows to apply avi or vca."""
