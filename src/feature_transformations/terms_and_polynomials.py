...
from cupyx.scipy.sparse import csc_matrix as csc_gpu, isspmatrix_csc


class SetsAVI:
    """
    Manages O, G, and borders.
    ...
    Methods:
        ...
        update_border_raw(border_terms_raw: cp.ndarray, border_evaluations_raw: cp.ndarray, non_purging_indices: list)
            Updates border_raw and non_purging_indices.
        ...
        update_O_indices(O_indices: list)
            Updates O_indices.
        update_G(G_coefficient_vectors: cp.ndarray, sparse_implementation: bool = False)
            Updates G.
        ...
        apply_G_transformation(X_test: cp.ndarray)
            Applies the transformation in G to X_test.
        reconstruct_border(O1_test: cp.ndarray,
                           O_test: cp.ndarray,
                           non_purging_indices: list,
                           X_test: cp.ndarray = None)
            Reconstrcuts the border of O_test.
        construct_border()
            Constructs the border of O_terms for the current state of the algorithm.
        evaluate_sparsity()
            Evaluates the sparsity of the polynomials in G_coefficient_vectors.
    ...
    def update_G(self, G_coefficient_vectors: cp.ndarray, sparse_implementation: bool = False):
        """Updates G."""
        if sparse_implementation:
            if G_coefficient_vectors is not None:
                G_coefficient_vectors = fd(G_coefficient_vectors)
                if fd(G_coefficient_vectors).shape[1] > 0:
                    if not isspmatrix_csc(G_coefficient_vectors):
                        G_coefficient_vectors = csc_gpu(G_coefficient_vectors)
                    if self.G_evaluations is None:
                        self.G_evaluations = (G_coefficient_vectors.T.dot((cp.hstack((
                            self.O_array_evaluations, self.border_evaluations_purged[-1]))).T)).T
                    else:
                        current_G_evaluations = (G_coefficient_vectors.T.dot((cp.hstack((
                            self.O_array_evaluations, self.border_evaluations_purged[-1]))).T)).T
                        self.G_evaluations = cp.hstack((cp.array(self.G_evaluations), cp.array(current_G_evaluations)))
    ...
    def evaluate_sparsity(self):
        ...
        Returns:
            total_number_of_zeros: int
                Sum of all zero entries in coeffecient vectors in G_coefficient_vectors.
            total_number_of_entries: int
                Total number of entries in coefficient vectors in G_coefficient_vectors.
            avg_sparsity: float
                The average sparsity of coefficient vectors in G_coefficient_vectors, where sparsity of a vector
                c = (c1, ..., ck)^T is defined as |{ci = 0}|/k.
            number_of_polynomials: int
                Number of polynomials in G.
        """
        number_of_polynomials, total_number_of_zeros, total_number_of_entries, avg_sparsity = 0, 0, 0, 0.0
        ...
        return total_number_of_zeros, total_number_of_entries, avg_sparsity, number_of_polynomials


class SetsVCA:
    """
    Manages the sets C, F, and V for VCA.

    Args:
        X: cp.ndarray
            The data train_or_test.
        m: int, Optional
    ...
    Methods:
        ...
        apply_V_transformation(X_test: cp.ndarray)
            Apply the transformation of the V polynomials to X_test.
        update_C(vectors: cp.ndarray)
            Update C.
        construct_border(degree: int = 1)
            Constructs the border for the current state of the algorithm.
        evaluate_sparsity()
            Evaluates the sparsity of the polynomials in V w.r.t. to the functions in F and C.
    ...
    def update_F(self, F_coefficient_vectors: cp.ndarray, F_evaluation_vectors: cp.ndarray):
        """Update F."""
        assert (isinstance(F_coefficient_vectors, cp.ndarray) or isspmatrix_csc(F_coefficient_vectors),
                "New Fs should not be empty.")
    ...
    def update_V(self, V_coefficient_vectors: cp.ndarray, V_evaluation_vectors: cp.ndarray):
        ...
        if isinstance(V_coefficient_vectors, cp.ndarray) or isspmatrix_csc(V_coefficient_vectors):
            V_vectors = fd(V_coefficient_vectors)
            assert (isinstance(V_evaluation_vectors, cp.ndarray) or isspmatrix_csc(V_coefficient_vectors),
                    "Evaluation vectors have to be cp.arrays.")
    ...
    def apply_V_transformation(self, X_test: cp.ndarray):
        ...
        while degree < len(self.Cs) + 1:
            ...
            if isinstance(V_coefficients_test, cp.ndarray) or isspmatrix_csc(V_coefficients_test):
            ...
            if isinstance(F_coefficients_test, cp.ndarray) or isspmatrix_csc(F_coefficients_test):
    ...
    def update_C(self, vectors: cp.ndarray):
        """Update C."""
        assert isinstance(vectors, cp.ndarray), "Requires cp.array."
    ...
    def evaluate_sparsity(self):
        ...
        Returns:
            total_number_of_zeros: int
                Sum of all zero entries in coeffecient vectors in V_coefficient_vectors.
            total_number_of_entries: int
            ...
            avg_sparsity: float
                ...
                c = (c1, ..., ck)^T is defined as |{ci = 0}|/k.
            number_of_polynomials: int
                Number of polynomials in G_coefficient_vectors.
        """
        total_number_of_zeros, total_number_of_entries, avg_sparsity = 0, 0, 0.0
