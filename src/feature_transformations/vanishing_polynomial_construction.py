...
def get_info(O_terms: cp.ndarray, border_terms: cp.ndarray, border_evaluations: cp.ndarray,
             coefficient_vectors: cp.ndarray):
    """Get info that has to be passed on.
...
def stabilized_reduced_row_echelon_form_algorithm(matrix: cp.ndarray, tau: float = 0.1):
    """Performs the Stabilized Reduced Row Echelon Form Algorithm.

    Brings matrix in SRREF and returns R and the column indices of the pivots.
...
def qr_decomposition(matrix: cp.ndarray, tau: float = 0.1):
    """Performs QR decomposition on matrix. Lines 1 -- 17 of Algorithm 20 [1].
...
def reduced_row_echelon_form_algorithm(matrix: cp.ndarray, tol: float = 1e-14):
    """
    Rescales rows. Lines 19 -- 24 of Algorithm 20 [2].
...
def find_range_null_vca(F: cp.ndarray, C: cp.ndarray, psi: float, sparse_implementation: bool = False):
    """
    ...
    Args:
        ...
        C: cp.ndarray
        psi: float
        sparse_implementation: bool, Optional
            (Default is False.)
    ...
    if sparse_implementation:
        if not isinstance(V_coefficient_vectors, list):
            V_coefficient_vectors = csc_gpu(V_coefficient_vectors)
        if not isinstance(F_coefficient_vectors, list):
            F_coefficient_vectors = csc_gpu(F_coefficient_vectors)
